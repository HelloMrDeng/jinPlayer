...
CDlgFileInfo::CDlgFileInfo(HINSTANCE hInst, HWND hParent)
	: m_hInst (hInst)
	, m_hParent (hParent)
	, m_hDlg (NULL)
	, m_hCmbFile (NULL)
...
CDlgFileInfo::~CDlgFileInfo(void)
{
	CloseFile ();
}

int CDlgFileInfo::OpenDlg (const TCHAR * pFile)
{
...
INT_PTR CALLBACK CDlgFileInfo::FileInfoDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	int				wmId, wmEvent;
	RECT			rcDlg;
	CDlgFileInfo *	pDlgInfo = NULL;

	if (hDlg != NULL)
	{
		GetClientRect (hDlg, &rcDlg);
		pDlgInfo = (CDlgFileInfo *)GetWindowLong (hDlg, GWL_USERDATA);
	}
	...
	case WM_INITDIALOG:
		...
		// pDlgInfo->GetFileInfo (_T("E:\\Temp\\aa.mp4"));
		// pDlgInfo->GetFileInfo (_T("E:\\Temp\\11.rmvb"));
		return (INT_PTR)TRUE;
...
int CDlgFileInfo::GetFileInfo (const TCHAR * pFile)
{
	int nRC = 0;
...
int CDlgFileInfo::FillStreamInfo (int nIndex)
{
...
int CDlgFileInfo::FillFileInfo (void)
{
	if (m_pInfoFile != NULL)
	{
		SetWindowText (m_hEdtInfo, m_pInfoFile);
...
void CDlgFileInfo::CloseFile (void)
{
	m_nReadStatus = YY_PLAY_Stop;
...
char *	CDlgFileInfo::GetLineText (char * pLine)
{
...
int CDlgFileInfo::GetFileName (TCHAR * pFileName)
{
	DWORD				dwID = 0;
	OPENFILENAME		ofn;
...
int CDlgFileInfo::GetHexString (unsigned char * pBuffer, int nBuffSize, int nLineSize, char * pText)
{
	char	szHex[8192];
	char	szLine[256];
	char	szWord[32];
...
long long CDlgFileInfo::BaseToTime (long long llBase, AVStream * pStream)
{
	return llBase * 1000 * pStream->time_base.num / pStream->time_base.den;
}

int CDlgFileInfo::ReadPacketProc (void * pParam)
{
	CDlgFileInfo * pDlgInfo = (CDlgFileInfo *)pParam;

	return pDlgInfo->ReadPacketLoop ();
}

int CDlgFileInfo::ReadPacketLoop (void)
{
	int			nRC = 0;
	int			i = 0;
	int			nIdx = 0;
...
bool CDlgFileInfo::IsRefFrame (char * buffer , int size)
{
...
int CDlgFileInfo::GetProfileName (AVCodecContext * pCodec, char * pInfo)
{
