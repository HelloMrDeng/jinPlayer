...
public class PlayerExt extends Activity implements SurfaceHolder.Callback,
	...
	private TextView 		m_tvPhoneTime;
...
//	private BasePlayer 		m_svrPlayer.m_Player = null;
	private	PlayerService	m_svrPlayer = null;
		...
		public void handleMessage(Message msg) 
		{
			...
			else if (msg.what == MSG_UPDATE_UI) 
			{
				...
				if (m_nDuration <= 0) {
					...
					m_nPos = (int) m_svrPlayer.m_Player.GetPos();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		m_svrPlayer = new PlayerService ();		
		m_svrPlayer.ResetPlayer();
		
        startService (new Intent (PlayerExt.this, PlayerService.class));	
		...
		if ((m_strVideoPath == null) || (m_strVideoPath.trim().length() <= 0)) {
			onError(m_svrPlayer.m_Player, 1, -1);
			return;
		}
	...
	private void initSystem() {
		m_amMain = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
	}
	...
	private void playVideo(String strPath) {
		...
		if (m_svrPlayer.m_Player == null)
			m_svrPlayer.CreatePlayer();
		//m_svrPlayer.m_Player = new BasePlayer();

		String apkPath = "/data/data/" + this.getPackageName() + "/lib/";
		m_svrPlayer.m_Player.SetView(m_svMain);
		nRet = m_svrPlayer.m_Player.Init(this, apkPath);

		if (nRet != 0) {
			onError(m_svrPlayer.m_Player, nRet, 0);
			return;
		}

		m_svrPlayer.m_Player.setEventListener(this);
		nRet = m_svrPlayer.m_Player.Open (strPath);
		if (nRet != 0) {
			onError(m_svrPlayer.m_Player, nRet, 0);
	...
	public void surfaceChanged(SurfaceHolder surfaceholder, int format, int w, int h) 
	{
		...
		{
			m_svrPlayer.m_Player.SetView(m_svMain);				
			m_svrPlayer.m_Player.SetParam(BasePlayer.PARAM_PID_SURFACE_CHANGED, 1);
		}
	}
	...
	{	
		...
		{
			m_svrPlayer.m_Player.SetView(m_svMain);					
			m_svrPlayer.m_Player.SetParam(BasePlayer.PARAM_PID_VIEW_ACTIVE, m_svMain);
			showMediaController();
			return;
		}

		Log.v(TAG, "The file is " + m_strVideoPath);
	...
	public void surfaceDestroyed(SurfaceHolder surfaceholder) 
	{
		if (m_svrPlayer.m_Player != null)
		{
			m_svrPlayer.m_Player.SetView(null);
		}
	}
	...
	public void PlayStop() {
		...
		if (m_svrPlayer.m_Player != null) {
			m_svrPlayer.m_Player.Stop();
			m_svrPlayer.m_Player.Uninit();
			m_svrPlayer.m_Player = null;
	...
	protected void onPause() 
	{
		...
		if (m_svrPlayer.m_Player != null) 
		{
			if (m_svrPlayer.m_Player.GetDuration() <= 0) 
			{
				PlayStop();
				finish();
			} 
			...
			{
				if (m_svrPlayer.m_Player.HasVideo() > 0)
				{
					m_svrPlayer.m_Player.Pause();
	...
	{
		Log.v(TAG, "onEvent ID is " + Integer.toHexString(nID) + "  obj is " + Integer.toHexString((Integer) obj));
		...
		if (nID == BasePlayer.YY_EV_Open_Complete) 
		{
			...
			m_nDuration = (int)m_svrPlayer.m_Player.GetDuration();
			
			String str = DateUtils.formatElapsedTime(m_nDuration / 1000);
			setTextFast(m_tvTotalTime, str);			
			
			onVideoSizeChanged (m_svrPlayer.m_Player.GetVideoWidth (), m_svrPlayer.m_Player.GetVideoHeight ());
			m_svrPlayer.m_Player.Play ();
		} 
	...
	{
		if ((m_svrPlayer.m_Player != null)) {
			m_svrPlayer.m_Player.SetPos(nPos);
		}
	}
