...
CWndSlider::CWndSlider(HINSTANCE hInst)
	...
	, m_nCurPos (0)
{
	_tcscpy (m_szDur, _T(" / 0:00:00"));
	_tcscpy (m_szPos, _T("0:00:00 / 0:00:00"));
}
...
bool CWndSlider::CreateWnd (HWND hParent, RECT rcView, COLORREF clrBG, bool bPopup)
{
	...
	SetRect (&m_rcThumb, 0, 0, 120, rcView.bottom);
	m_hBrushTmb = ::CreateSolidBrush (RGB (200, 200, 200));
	SetRect (&m_rcText, 0, 10, 120, rcView.bottom);
...
bool CWndSlider::SetRange (int nMin, int nMax)
{
	...
	int nDur = (int)(m_nMaxPos / 1000);
	_stprintf (m_szDur, _T("%1d:%02d:%02d"), nDur / 3600, (nDur % 3600) / 60, nDur % 60);
	return true;
}
...
int CWndSlider::SetPos (int nPos)
{
	...
	m_nCurPos = nPos;
	nPos = nPos / 1000;
	_stprintf (m_szPos, _T("%1d:%02d:%02d / %s"), nPos / 3600, (nPos % 3600) / 60, nPos % 60, m_szDur);
...
LRESULT CWndSlider::OnReceiveMessage (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		SetRect (&m_rcText, m_nThumbPos, m_rcText.top, m_nThumbPos + m_rcThumb.right, m_rcText.bottom);
		DrawText (hdc, m_szPos, _tcslen (m_szPos), &m_rcText, DT_CENTER | DT_VCENTER);
