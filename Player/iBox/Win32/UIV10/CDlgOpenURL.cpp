...
INT_PTR CALLBACK CDlgOpenURL::OpenURLDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowText (hDlg, pDlgURL->m_pLangText->GetText (YYTEXT_OpenURL));
		SetWindowText (GetDlgItem (hDlg, IDC_BUTTON_OPEN), pDlgURL->m_pLangText->GetText (YYTEXT_Open));
		SetWindowText (GetDlgItem (hDlg, IDC_BUTTON_ADD), pDlgURL->m_pLangText->GetText (YYTEXT_Add));
		SetWindowText (GetDlgItem (hDlg, IDC_BUTTON_DELETE), pDlgURL->m_pLangText->GetText (YYTEXT_Delete));
	...
	case WM_COMMAND:
		...
		{
			int nSel = SendMessage (pDlgURL->m_hListURL, LB_GETSEL, 0, 0);
			if (nSel >= 0)
				SendMessage (pDlgURL->m_hListURL, LB_DELETESTRING, nSel, 0);
			break;
		}
