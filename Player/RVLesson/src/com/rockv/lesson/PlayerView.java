...
public class PlayerView extends Activity 
	...
	private void OpenFile (String strPath) {
		...
		m_Player.SetParam(m_Player.PARAM_PID_COLOR_FORMAT, 1, null);	
		m_Player.SetParam(BasePlayer.PARAM_PID_VIDEODEC_MODE,  BasePlayer.YY_VDMODE_Soft,  null);
		if (strPath.substring(0, 5).endsWith("file:")) {
			m_strPDP = Environment.getExternalStorageDirectory().getPath () + "/RockVL/Download/";
			int 	nPos = strPath.length() - 5;
			char 	chPos;
			while (nPos > 0) {
				chPos = strPath.charAt(nPos--);
				if (chPos == '/') {
					m_strPDP = m_strPDP + strPath.substring(nPos+2);;
					break;
				}
			}			
			m_Player.SetParam(BasePlayer.PARAM_PID_PDP_FILE,  0,  m_strPDP);
		}
